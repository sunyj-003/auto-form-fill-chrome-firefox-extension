...
浏览器扩展 (Chrome MV3)，使用逼真的假数据填充网页表单。支持文本输入、下拉框、复选框、单选按钮和文件上传。
...
- 这是一个通用表单填充扩展，任何实现都不能依赖域名、路径或单站点特判。
- 识别优先级是“结构识别”先于“字段语义猜测”。
- 现代前端框架下的虚拟控件必须先还原成统一控件模型，再决定如何填充。
- 对下拉框、日期控件、组合输入等框架组件，优先做结构级适配，不把它们当普通文本框处理。
- 对静态 DOM 无法表达的输入约束，优先通过运行时探测判断控件实际接受什么值。
- 填充值决策应尽量依赖机器可读信号和运行时行为，不依赖站点文案或单页上下文做脆弱推断。
- 低置信度字段默认跳过，宁可少填，也不要填错。
- 任何新增规则都应优先抽象为通用能力；如果某个规则只能解释单一网站，它就不应该进入核心逻辑。
